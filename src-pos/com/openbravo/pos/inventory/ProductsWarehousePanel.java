...
public class ProductsWarehousePanel extends JPanelTable {
    ...
    /** Creates a new instance of ProductsWarehousePanel */
     ...
     protected void init() {
        ...
        lpr = new ListProviderCreator(new PreparedSentence(app.getSession(),
                "SELECT PRODUCTS.ID, PRODUCTS.REFERENCE, PRODUCTS.NAME, ?, S.STOCKSECURITY, S.STOCKMAXIMUM, COALESCE(S.UNITS, 0) " +
                "FROM PRODUCTS LEFT OUTER JOIN " +
                "(SELECT PRODUCT, LOCATION, STOCKSECURITY, STOCKMAXIMUM, UNITS FROM STOCKCURRENT WHERE LOCATION = ?) S ON PRODUCTS.ID = S.PRODUCT " +
    ...
    public ListCellRenderer getListCellRenderer() {
        return new ListCellRendererBasic(new RenderStringRefName(new Formats[] {Formats.STRING, Formats.STRING, Formats.STRING}, new int[]{1, 2}));
    }
