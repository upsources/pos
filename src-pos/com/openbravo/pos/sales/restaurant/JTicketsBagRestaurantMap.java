...
//    private static final Icon ICO_FRE = new NullIcon(22, 22);
    ...
    public JTicketsBagRestaurantMap(AppView app, TicketsEditor panelticket) {
        ...
        try {
            SentenceList sent = dlSales.getSentFloorsList();
            m_afloors = sent.list();
        } catch (BasicException eD) {
            m_afloors = new ArrayList<Floor>();
        }
        try {
            SentenceList sent = dlSales.getPositionFloorsList();
            m_aplaces = sent.list();
        } catch (BasicException eD) {
        ...
        if (m_PlaceClipboard == null) {
            if (customer == null) {
                ...
                for (Place place : m_aplaces) {
                    if (place.hasPeople()) {
                        try {
                            AppUser userId = m_App.getAppUserView().getUser();
                            String receiptUserId = dlReceipts.getUserId(place.getId());
                            if ( ( userId != null && receiptUserId != null && receiptUserId.equals( userId.getUserInfo().getId() ) ) || userId.hasPermission("sales.MoveAnyTable")) {
    ...
    private TicketInfo getTicketInfo(Place place) {
        ...
        try {
            ...
            ticket = dlReceipts.getSharedTicket(place.getId());
            // Get ticket user if any, on table move, user should be preserved!
            userId = dlReceipts.getUserId(place.getId());
            if( userId != null && ticket != null )
                ticket.setUser( new UserInfo( userId, "") );
